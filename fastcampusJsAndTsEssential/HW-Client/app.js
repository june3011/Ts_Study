  ...
  for (let i = (store.currentPage - 1) * 10; i < store.currentPage * 10; i++) {
    newsList.push(`
        <div class="p-6 bg-white mt-6 rounded-lg shadow-md transition-colors duration-500 hover:bg-green-100">
            <div class="flex">
                <div class="flex-auto">
                    <a href="#/show/${newPeed[i].id}">${newPeed[i].title}</a>
                </div>
                <div class="text-center text-sm">
                    <div class="w-10 text-white bg-green-300 rounded-lg px-0 py-2">${newPeed[i].comments_count}</div>
                </div>
            </div>
            <div class="flex mt-3">
                <div class="grid grid-cols-3 text-sm text-gray-500">
                    <div><i class="fas fa-user mr-1"></i>${newPeed[i].user}</div>
                    <div><i class="fas fa-user mr-1"></i>${newPeed[i].points}</div>
                    <div><i class="fas fa-user mr-1"></i>${newPeed[i].time_ago}</div>
                </div>
            </div>
        </div>
    `);
