...
class NewsFeedView extends View {
  ...
  render(): void {
    for (
      ...
      const { id, title, comments_count, user, positions, time_ago, read } =
        this.feeds[i];
      this.addHtml(`
        <div class="p-6 ${
          read ? "bg-red-500" : "bg-white"
            ...
            <div class="flex mt-3">
                <div class="grid grid-cols-3 text-sm text-gray-500">
                    ...
                    <div><i class="fas fa-user mr-1"></i>${positions}</div>
                    <div><i class="fas fa-user mr-1"></i>${time_ago}</div>
                </div>
            </div>
        </div>
    `);
    ...
    this.setTemplateData(
      `next_page`,
      String(
        this.feeds.length / 10 > store.currentPage
          ? store.currentPage + 1
          : store.currentPage
      )
    );
